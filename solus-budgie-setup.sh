...
sudo eopkg it intel-microcode mc vim psensor htop xpad p7zip dia flameshot  simplenote freeplane liferea pidgin  openjdk-8 openjdk-8-devel openjfx-8 ruby r fpc sed gawk git git-extras git-cola python-ipython python3-ipython scipy numpy matplotlib python-seaborn texlive-all gnome-latex gnome-latex-docs geany geany-plugins geany-themes codeblocks bluefish vscode zsh openssh fish  budgie-shutdown-timer zim libreoffice-all google-chrome-stable opera-stable thunderbird etcher bookworm  desmume dolphin-emu dolphin-emu dosbox mgba snes9x-gtk VisualBoyAdvance-M -y

# sudo eopkg it dropbox texlive okular texstudio calibre retroarch pcsx2 pcsxr ppsspp
